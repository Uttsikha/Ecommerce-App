<?php include '../includes/db.php';
	...
	if (isset($_REQUEST['order_return_status'])){
		$up_sql1="UPDATE orders SET order_return_status= '$_REQUEST[order_return_status]' WHERE order_id = '$_REQUEST[order_id]'";
		$up_run=mysqli_query($conn,$up_sql1);
	}
?>
		...
		<?php
			...
			while ($rows= mysqli_fetch_assoc($run)){
				...
				<tr>
					...
					<td><button class='btn btn-info' data-toggle='modal' data-target='#order_chk_modal$rows[order_id]'>
					$rows[order_checkout_ref]</button>
								...
								<div class='modal-body'>
									<table class='table'>
										...
										while ($chk_rows=mysqli_fetch_assoc($chk_run)){
											...
											$total=$chk_rows['chk_qty'] * $chk_rows['item_price'];
											echo"
											<tr>
												<td>1</td>
												<td>$item_title</td>
